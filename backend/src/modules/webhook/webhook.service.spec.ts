...
describe('WebhookService', () => {
  ...
  it('rejeita token inválido com 401 e NÃO chama o n8n', async () => {
    const svc = new WebhookService();
  ...
  it('rejeita token ausente (undefined) com 401', async () => {
    const svc = new WebhookService();
    await expect(svc.repassarNina({}, undefined)).rejects.toBeInstanceOf(
      UnauthorizedException,
    );
    expect(fetchMock).not.toHaveBeenCalled();
  });

  it('503 quando NINA_WEBHOOK_TOKEN não está configurado', async () => {
    delete process.env.NINA_WEBHOOK_TOKEN;
    const svc = new WebhookService();
    await expect(svc.repassarNina({}, 'qualquer')).rejects.toBeInstanceOf(
      ServiceUnavailableException,
    );
    expect(fetchMock).not.toHaveBeenCalled();
  });

  it('503 quando NINA_N8N_WEBHOOK_URL não está configurado', async () => {
    delete process.env.NINA_N8N_WEBHOOK_URL;
    const svc = new WebhookService();
    await expect(svc.repassarNina({}, TOKEN)).rejects.toBeInstanceOf(
      ServiceUnavailableException,
    );
    expect(fetchMock).not.toHaveBeenCalled();
  });
