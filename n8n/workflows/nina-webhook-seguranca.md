...
> ✅ **Estado (2026-06-25):** **PUBLICADO e ativo** em produção (`activeVersionId 816107b0…`,
> `versionId == activeVersionId`, 46 nós). Como é **fail-open** e o segredo ainda **não** está
> setado, o nó **passa tudo** — comportamento idêntico ao anterior. Vira **fail-closed** sozinho
> assim que o segredo existir no ambiente do n8n. (Publish já feito pela Claude via MCP.)
...
> O nó **já está publicado** (fail-open). Os passos abaixo só **ativam** a checagem.
...
2. **Evolution PRIMEIRO** — configurar o webhook da instância `nina` para enviar o header
   ...
   versão não tiver, use a alternativa do path — ver abaixo). O nó já está publicado mas
   fail-open: o header chega e é **ignorado** → **nada quebra**.
3. **Validar** que mensagens reais continuam chegando (manda um self-chat "nina ping").
4. **Setar `EVOLUTION_WEBHOOK_HMAC_SECRET=<segredo>`** no ambiente do **serviço n8n** (EasyPanel)
   + **Restart** do n8n (lê env no boot). Agora o nó está **fail-closed**.
5. **Validar fail-closed (teste de aceite):**
